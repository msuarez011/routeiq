      ...
      <form onSubmit={handleSearch} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
            ...
            <span style={{ fontSize: 14, color: position ? 'var(--color-text-primary)' : 'var(--color-text-muted)' }}>
              {position ? `Mi ubicación (${position.lat.toFixed(4)}, ${position.lng.toFixed(4)})` : 'Obteniendo ubicación...'}
            </span>
          </div>
          <div style={{ height: '0.5px', background: 'var(--color-border)', marginBottom: 12 }} />
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ width: 8, height: 8, borderRadius: '50%', background: 'var(--color-dest)', flexShrink: 0 }} />
            <input
